...
export async function deleteLocalBatch(
  ...
  for (let i = 0; i < createdTasks.length; i++) {
    ...
    sftpTransferStore.mutateAllTaskNodes(task.id, 'transferring', { startTime: Date.now() }, 'delete-local')
      ...
      try {
        ...
        // 批量更新所有子节点状态为 completed（与 transferring 对称）
         sftpTransferStore.mutateAllTaskNodes(task.id, 'completed', { progress: 100, speed: 0, endTime: Date.now() }, 'delete-local')
