...
## P2 - listDir 全量加载目录 ⏳ 暂不修复

> 📋 核查日期: 2026-04-26 | 位置: [listDir](../sftp.ts#L93)

**状态**: ~~待处理~~ → **经核查决定暂不修复**

**原问题描述**: `readdir` 一次性读取整个目录所有条目到内存。

**暂不修复原因**:
1. **ssh2 库限制**：Node.js 的 ssh2 库只封装了 SFTP v3 基础操作集，未暴露 `OPENDIR`/`READDIR` 分页 API，无法在应用层实现分批读取
2. **实际风险可控**：用户浏览的目录通常不超过几百条目；真正上万文件的目录极少通过 SFTP 浏览器打开
3. **收益成本比低**：要实现需底层改用其他 SFTP 库或自行封装协议扩展，工程改动大但用户体验提升不明显

**替代方案（已实施）**: 在 UI 层添加骨架屏加载组件，提升大目录（如 `/bin`）访问时的等待体验。
