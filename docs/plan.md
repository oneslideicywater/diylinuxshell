...
| Phase 2 | V1.1 | 密钥认证、会话分组、命令片段、主题配置、功能测试 | 1周 |
| Phase 3 | V1.2 | 终端分屏、日志记录、历史记录、导入导出、功能测试 | 1周 |
| Phase 4 | V2.0 | SFTP集成、跳板机、批量操作、脚本执行、功能测试 | 2周 |
...
| 2.3 | 实现会话分组功能 | SessionGroup.vue | Phase 1 |
| 2.4 | 实现命令片段存储 | store.ts 扩展 | Phase 1 |
| 2.5 | 实现命令片段管理UI | CommandSnippet.vue | 2.4 |
| 2.6 | 实现主题配置 | themes/*.css | Phase 1 |
| 2.7 | 实现设置页面 | Settings.vue | 2.6 |
| 2.8 | 实现ConfigStore | config.ts | 2.6 |
| 2.9 | 编写密钥认证集成测试 | key-auth.integration.test.ts | 2.1 |
| 2.10 | 编写会话分组单元测试 | session-group.test.ts | 2.3 |
| 2.11 | 编写命令片段单元测试 | command.test.ts | 2.4 |
| 2.12 | 编写ConfigStore单元测试 | config.test.ts | 2.8 |
| 2.13 | 编写命令片段E2E测试 | command.e2e.spec.ts | 2.5 |
| 2.14 | 编写主题切换E2E测试 | theme.e2e.spec.ts | 2.6 |
