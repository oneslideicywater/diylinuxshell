...
| 阶段 | 名称 | 核心目标 | 预计工期 | 状态 |
|------|------|----------|----------|------|
| Phase 0 | 项目初始化 | 搭建项目骨架、配置开发环境、配置打包、配置测试框架 | 1 天 | ✅ 已完成 |
| Phase 1 | V1.0 MVP | 基础 SSH 连接、会话管理、多标签页、密码认证、核心测试 | 1 周 | ✅ 已完成（2026-04-25） |
| Phase 2 | V1.1 | 密钥认证、会话分组、SFTP 传输、主题配置、功能测试 | 1 周 | ✅ 已完成（2026-04-25） |
| Phase 3 | V1.3 AI 助理 | AI 对话交互、终端输出分析、智能文件操作、组合式任务、功能测试 | 2 周 | ⏳ 待开始 |
| Phase 4 | V1.2 高级功能 | 终端分屏、日志记录、历史记录、导入导出、命令片段、功能测试 | 1 周 | ⏳ 待开始 |
| Phase 5 | V2.0 专业功能 | 跳板机、批量操作、脚本执行、功能测试 | 2 周 | ⏳ 待开始 |
| Phase 6 | 发布部署 | 多平台打包、CI/CD、代码签名、全量测试、发布 | 3 天 | ⏳ 待开始 |
...
- [x] 终端能够正确显示输出内容
- [x] 多标签页功能正常工作
- [x] 切换标签页时终端内容正确切换（每个标签页独立终端实例）
- [x] vi/vim 编辑器正常使用

**测试覆盖**：
- ✅ E2E 测试：connection.e2e.spec.ts, vim.e2e.spec.ts, tabs.e2e.spec.ts
- ✅ 单元测试：crypto.test.ts, store.test.ts, ssh-manager.test.ts, session.test.ts, terminal.test.ts
- ✅ 集成测试：session.integration.test.ts, terminal.integration.test.ts
...
| 2.25 | 实现 SFTP 文件传输 | SftpWindow.vue, sftp.ts | Phase 1 |
| 2.26 | 实现 SFTP 右键菜单 | sftp-context-menu.ts | 2.25 |
| 2.27 | 实现文件夹上传/下载 | sftp-recursive.ts | 2.25 |
| 2.28 | 实现进度状态栏 | SftpStatusBar.vue | 2.25 |
| 2.29 | 实现树形传输状态 | SftpTransferTree.vue | 2.25 |
| 2.30 | 实现取消传输功能 | sftp-cancel.ts | 2.25 |
| 2.31 | 实现右键菜单互斥 | context-menu-manager.ts | 2.25 |
| 2.32 | 实现 SSH/SFTP 模式切换 | terminal-tab-store.ts | Phase 1 |
| 2.33 | 实现侧边栏宽度调整 | SessionList.vue | Phase 1 |
| 2.34 | 实现嵌套分组层级限制 | session-group-utils.ts | 2.3 |
| 2.35 | 实现菜单项高亮效果 | context-menu.scss | 2.3 |
| 2.36 | 实现 L 型连接线 | session-group.scss | 2.3 |
| 2.37 | 实现树形分组选择器 | GroupTreePicker.vue | 2.3 |
| 2.38 | 实现测试连接功能 | test-connection.ts | 2.1 |
| 2.39 | 编写 SFTP E2E 测试 | sftp-*.e2e.spec.ts | 2.25-2.31 |
| 2.40 | 编写 SSH/SFTP 模式切换测试 | ssh-sftp-mode-switch.e2e.spec.ts | 2.32 |
| 2.41 | 编写侧边栏调整 E2E 测试 | app-sidebar-resize.e2e.spec.ts | 2.33 |
| 2.42 | 编写五层嵌套分组测试 | five-level-nested-groups.e2e.spec.ts | 2.34 |
...
6. 测试验证
   └── 2.17-2.24, 2.39-2.42 功能测试与 E2E 验证
```
...
**测试覆盖**：
- ✅ E2E 测试：40+ 个测试用例
  - 会话分组：13+ 个（five-level-nested-groups, group-contextmenu-bugfix, etc.）
  - SFTP 传输：19 个（sftp-transfer, sftp-tree-upload, cancel-upload, etc.）
  - 侧边栏调整：7 个（app-sidebar-resize 系列）
  - SSH/SFTP 模式切换：1 个
- ✅ 单元测试：config.test.ts, key-auth.integration.test.ts
