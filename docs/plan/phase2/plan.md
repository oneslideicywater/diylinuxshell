...
9. **L 型连接线** - 完全实现 ✅
   - ✅ 垂直线段和水平线段
   - ✅ 线条样式（1px，var(--el-border-color-light)）
...
10. **SFTP 文件传输功能** - 完全实现 ✅
    ...
    - ✅ 进度状态栏（实时显示上传/下载进度）
    - ✅ 树形传输状态显示（类似 Xshell）
      - ✅ SftpTransferTree 树形组件
      - ✅ TreeNode 递归节点组件
      - ✅ SftpStatusBar 集成树形面板
      - ✅ 展开/折叠功能
      - ✅ 状态颜色标识（传输中/已完成/失败）
      - ✅ 删除进度树形显示
    - ✅ 主题适配（深色/浅色）
    ...
    - ✅ 右键菜单互斥显示（全局唯一）
    - ✅ 取消传输功能
    - ✅ E2E 测试全部通过（19 个 SFTP 测试）
11. **SSH/SFTP 模式切换** - 完全实现 ✅
    - ✅ 标签页模式切换按钮
    - ✅ Pinia Store 同步切换
    - ✅ 终端组件独立实例

**完成状态**：✅ Phase 2 已全部完成（2026-04-25）
...
- [x] **树形分组选择器**
  ...
  - [x] 滚动支持（分组过多时）
- [x] **E2E 测试覆盖**
  - [x] 五层嵌套分组测试（five-level-nested-groups.e2e.spec.ts）
  - [x] 分组上下文菜单测试（group-contextmenu-bugfix.e2e.spec.ts）
  - [x] 分组重构验证测试（group-refactoring-validation.e2e.spec.ts）
  - [x] 子分组缩进测试（subgroup-indent.e2e.spec.ts）
  - [x] 会话分组基础测试（session-group.e2e.spec.ts）
...
- [x] E2E 测试全部通过（7 个测试用例：app-sidebar-resize.e2e.spec.ts）

### SFTP 文件传输（已完成 ✅）
- [x] SFTP 窗口双面板布局
- [x] 本地文件列表显示和刷新
- [x] 远程文件列表显示和刷新
- [x] 文件上传功能（单个文件、文件夹递归上传）
- [x] 文件下载功能（单个文件、文件夹递归下载）
- [x] 新建文件夹功能
- [x] 删除文件功能（单个删除、批量删除）
- [x] 进度状态栏（实时显示上传/下载进度）
...
- [x] 右键菜单互斥显示（全局唯一）
- [x] 工具栏右键菜单
- [x] E2E 测试全部通过（19 个 SFTP 测试用例）

### SSH/SFTP 模式切换（已完成 ✅）
- [x] 标签页模式切换按钮
- [x] Pinia Store 同步切换逻辑
- [x] 终端组件独立实例管理
- [x] E2E 测试（ssh-sftp-mode-switch.e2e.spec.ts）
